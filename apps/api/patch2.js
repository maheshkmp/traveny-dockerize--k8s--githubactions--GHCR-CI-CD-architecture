...
const targetRegex = /\{\/\*\s*Facebook Direct Publish Component\s*\*\/\}\s*\{formatId === "facebook" && \(\s*<div className="pt-2 border-t border-border mt-4">/g;

const replacement = `{/* Facebook Direct Publish Component */}
                      {formatId === "facebook" && (
...
if (data.match(targetRegex)) {
  data = data.replace(targetRegex, replacement);

  const btnRegex = /\) : \(\s*<Button\s*onClick={publishToFacebook}/g;
  ...
  data = data.replace(btnRegex, btnReplacement);

  const closingDivRegex = /<\/Button>\s*\)\}\s*<\/div>\s*\)\}\s*<\/div>/g;
  const closingDivReplacement = `</Button>\n                              </div>\n                            )}\n                          </div>\n                        )}\n                      </div>`;
  data = data.replace(/<\/Button>\n\s*\)\}\n\s*<\/div>\n\s*\)\}\n\s*<\/div>/g, closingDivReplacement);
